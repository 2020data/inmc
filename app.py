這是一個非常實用且注重細節的升級！為了達到您的需求，我做了以下三個關鍵調整：

1. **高對比文字設計**：加深了姓名與名次的文字顏色（使用 `#111111` 深黑與高飽和度的主題色），並調整了陰影效果，讓印出來的獎狀文字更加霸氣且層次分明。
2. **三種預設背景**：除了自訂上傳，我使用 CSS 漸層設計了三款質感背景：「經典象牙白」、「典雅曜金漸層」與「星空湛藍漸層」，讓您不須上傳圖片也能立刻擁有精緻的獎狀外觀。
3. **Top 25% 數量下限保護**：將計算邏輯修改為 `q1_count = min(總人數, max(3, math.ceil(總人數 * 0.25)))`。這能確保只要參賽者超過 3 人（或 3 組），即使 25% 算出來不到 3，也**強制保障發放前三名**的優選獎狀。

以下是為您更新的完整程式碼，您可以直接覆蓋原本的檔案使用：
